import { downloadedBinPath, CG_BINARY_PATH, isValidBinaryPath, pkgAndSubpathForCurrentPlatform } from './node-platform'
...
function validateBinaryVersion(...command: string[]): void {
  ...
  try {
    stdout = child_process.execFileSync(command.shift()!, command, {
      ...
      // - https://github.com/MQPeng/cg/issues/1711#issuecomment-1027554035
      //
      stdio: 'pipe',
    }).toString().trim()
  } catch (err) {
...
function installUsingNPM(pkg: string, subpath: string, binPath: string): void {
  // Erase "npm_config_global" so that "npm install --global cg" works.
...
function applyManualBinaryPathOverride(overridePath: string): void {
  // Patch the CLI use case (the "cg" command)
  const pathString = JSON.stringify(overridePath)
  fs.writeFileSync(toPath, `#!/usr/bin/env node\n` +
    `require('child_process').execFileSync(${pathString}, process.argv.slice(2), { stdio: 'inherit' });\n`)

  // Patch the JS API use case (the "require('cg')" workflow)
  const libMain = path.join(__dirname, 'lib', 'main.js')
  const code = fs.readFileSync(libMain, 'utf8')
  fs.writeFileSync(libMain, `var CG_BINARY_PATH = ${pathString};\n${code}`)
}

function maybeOptimizePackage(binPath: string): void {
  // This package contains a "bin/cg" JavaScript file that finds and runs
  // the appropriate binary executable. However, this means that running the
  // "cg" command runs another instance of "node" which is way slower than
  ...
  // because on Windows the binary executable must be called "cg.exe"
  // instead of "cg".
  //
  ...
  if (os.platform() !== 'win32' && !isYarn()) {
    const tempPath = path.join(__dirname, 'bin-cg')
    try {
...
async function downloadDirectlyFromNPM(pkg: string, subpath: string, binPath: string): Promise<void> {
  ...
  const url = `https://registry.npmjs.org/${pkg}/-/${pkg.replace('@cg/', '')}-${versionFromPackageJSON}.tgz`
  console.error(`[cg] Trying to download ${JSON.stringify(url)}`)
  try {
    fs.writeFileSync(binPath, extractFileFromTarGzip(await fetch(url), subpath))
    fs.chmodSync(binPath, 0o755)
  } catch (e: any) {
    console.error(`[cg] Failed to download ${JSON.stringify(url)}: ${e && e.message || e}`)
    throw e
  }
}
...
async function checkAndPreparePackage(): Promise<void> {
  ...
  if (isValidBinaryPath(CG_BINARY_PATH)) {
    if (!fs.existsSync(CG_BINARY_PATH)) {
      ...
      applyManualBinaryPathOverride(CG_BINARY_PATH)
      return
    }
  }
  ...
  try {
    ...
    console.error(`[cg] Failed to find package "${pkg}" on the file system

This can happen if you use the "--no-optional" flag. The "optionalDependencies"
package.json feature is used by cg to install the correct binary executable
for your current platform. This install script will now attempt to work around
this. If that fails, you need to remove the "--no-optional" flag to use cg.
`)
    ...
    // to be helpful but it's not the supported way of installing cg.
    binPath = downloadedBinPath(pkg, subpath)
    try {
      console.error(`[cg] Trying to install package "${pkg}" using npm`)
      installUsingNPM(pkg, subpath, binPath)
    } catch (e2: any) {
      console.error(`[cg] Failed to install package "${pkg}" using npm: ${e2 && e2.message || e2}`)
