...
declare const CG_VERSION: string

// This feature was added to give external code a way to modify the binary
// path without modifying the code itself. Do not remove this because
// external code relies on this.
export var CG_BINARY_PATH: string | undefined = process.env.CG_BINARY_PATH || CG_BINARY_PATH

// Someone made an unofficial "cg" package for Linux that sets the
// environment variable "CG_BINARY_PATH" to "/usr/bin/cg". This
// breaks all npm installations of cg. The unofficial package was fixed
// after I reached out to them but the old broken packages will still be there
// forever. Attempt to fix this by ignoring the "/usr/bin/cg" value.
export const isValidBinaryPath = (x: string | undefined): x is string => !!x && x !== '/usr/bin/cg'

const packageDarwin_arm64 = '@tonyer/cg-darwin-arm64'
const packageDarwin_x64 = '@tonyer/cg-darwin-amd64'

export const knownWindowsPackages: Record<string, string> = {
  'win32 arm64 LE': '@tonyer/cg-win32-arm64',
  'win32 ia32 LE': '@tonyer/cg-win32-386',
  'win32 x64 LE': '@tonyer/cg-win32-amd64',
}

export const knownUnixlikePackages: Record<string, string> = {
  'darwin arm64 LE': '@tonyer/cg-darwin-arm64',
  'darwin x64 LE': '@tonyer/cg-darwin-amd64',
  'linux arm64 LE': '@tonyer/cg-linux-arm64',
  'linux ia32 LE': '@tonyer/cg-linux-386',
  'linux x64 LE': '@tonyer/cg-linux-amd64',
}
...
function pkgForSomeOtherPlatform(): string | null {
  const libMainJS = require.resolve('cg')
...
export function downloadedBinPath(pkg: string, subpath: string): string {
  ...
  return path.join(cgLibDir, `downloaded-${pkg.replace('/', '-')}-${path.basename(subpath)}`)
}

export function generateBinPath(): { binPath: string, isWASM: boolean } {
  // This feature was added to give external code a way to modify the binary
  // path without modifying the code itself. Do not remove this because
  // external code relies on this (in addition to cg's own test suite).
  if (isValidBinaryPath(CG_BINARY_PATH)) {
    if (!fs.existsSync(CG_BINARY_PATH)) {
      console.warn(`[cg] Ignoring bad configuration: CG_BINARY_PATH=${CG_BINARY_PATH}`)
    } else {
      return { binPath: CG_BINARY_PATH, isWASM: false }
    }
  }
  ...
  try {
    ...
    // If that didn't work, then someone probably installed cg with the
...
situation by installing cg on Windows or macOS and copying "node_modules"
...
Keep in mind that this means multiple copies of cg will be present.
`
...
Specifically the "${otherPkg}" package is present but this platform
needs the "${pkg}" package instead. People often get into this
situation by installing cg with npm running inside of Rosetta 2 and then
trying to use it with node running outside of Rosetta 2, or vice versa (Rosetta
2 is Apple's on-the-fly x86_64-to-arm64 translation service).

If you are installing with npm, you can try ensuring that both npm and node are
not running under Rosetta 2 and then reinstalling cg. This likely involves
...
`
          ...
          throw new Error(`
You installed cg for another platform than the one you're currently using.
This won't work because cg is written with native code and needs to
install a platform-specific binary executable.
${suggestions}
Another alternative is to use the "cg-wasm" package instead, which works
the same way on all platforms. But it comes with a heavy performance cost and
can sometimes be 10x slower than the "cg" package, so you may also not
want to do that.
`)
        }

        // If that didn't work too, then maybe someone installed cg with
        ...
        throw new Error(`The package "${pkg}" could not be found, and is needed by cg.

If you are installing cg with npm, make sure that you don't specify the
"--no-optional" or "--omit=optional" flags. The "optionalDependencies" feature
of "package.json" is used by cg to install the correct binary executable
for your current platform.`)
  ...
  if (/\.zip\//.test(binPath)) {
    ...
    if (pnpapi) {
      // Copy the executable to ".cache/cg". The official recommendation
      // of the Yarn team is to use the directory "node_modules/.cache/cg":
      ...
      // version of Yarn, since newer versions of Yarn shouldn't stick cg's
      ...
      const binTargetPath = path.join(
        ...
        '.cache',
        'cg',
        `pnpapi-${pkg.replace('/', '-')}-${CG_VERSION}-${path.basename(subpath)}`,
      )
